...
; ===============================
GetSafeGuiPos(ByRef outX, ByRef outY
            , guiW := 0, guiH := 0
            , rows := 1, cols := 1) {
    ...
    ; ===== konfigurasi HARUS sama dengan GUI =====
    ButtonW      := 50
    ButtonMargin := 2
    GuiPadding   := 20

    ; ===== hitung lebar popup berdasarkan kolom nyata =====
    popupW := (cols * ButtonW)
            + ((cols - 1) * ButtonMargin)
            + GuiPadding
    ...
    ; ===============================
    ; HITUNG X DINAMIS (AKURAT)
    ; ===============================
    if (hasCaret) {
        rawX := pos.x

        overflow := (rawX + popupW) - (screenW - margin)
    ...
    if (hasCaret) {
        ...
        offsetY := (rows <= 1) ? oneRowOffset : multiRowOffset
...
ShowNumPopup:
	...
	cols := (row = 1) ? (col - 1) : MaxCols
	
    Gui, num:Show, AutoSize Hide
	WinGetPos,,, guiW, guiH, A
	GetSafeGuiPos(guiX, guiY, guiW, guiH, row, cols)
